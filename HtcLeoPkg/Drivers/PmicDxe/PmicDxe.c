...
static int pm8058_masked_write(UINT16 addr, UINT8 val, UINT8 mask)
{
	int rc;
	UINT8 reg;
 ...
 */
static int disable_smps_locally_set_pull_down(UINT16 ctrl_addr, UINT16 test2_addr,
...
static int disable_ldo_locally_set_pull_down(UINT16 ctrl_addr,UINT16 master_enable_addr, UINT8 master_enable_bit)
{
	int rc;
...
static int pm8058_read_buf(UINT16 addr, UINT8 *buf,int cnt)
{

	return gSsbi->SsbiRead(addr, buf, cnt);
}

static int pm8058_write_buf(UINT16 addr, UINT8 *buf,int cnt)
{

	return gSsbi->SsbiWrite(addr, buf, cnt);
}

static enum pm8xxx_version pm8058_get_version(void)
{
...
static int pm8058_get_revision(void)
{
