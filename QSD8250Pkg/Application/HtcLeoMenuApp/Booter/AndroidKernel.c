    ...
    unsigned ramdisk_size)
{
    ...
    *ptr++ = 0;
    ...
    DEBUG((EFI_D_ERROR, "booting linux @ %p, ramdisk @ %p (%d)\n", kernel, ramdisk, ramdisk_size));
    if (cmdline)
    {
        CHAR16 CmdlineUnicode[256];
        AsciiStrToUnicodeStr(cmdline, CmdlineUnicode);
        Print(L"cmdline: %s\n", CmdlineUnicode);
        DEBUG((EFI_D_ERROR, "cmdline: %s\n", CmdlineUnicode));
    }
    ...
    IN CHAR16 *KernelFileName)
{
    ...
    Status = Root->Open(
        Root,
        &BootFile, // Assign the global BootFile here
        KernelFileName,
        EFI_FILE_MODE_READ,
        0);
    if (EFI_ERROR(Status))
...
void BootAndroidKernel(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    DEBUG((EFI_D_ERROR, "BootDir is %s\n", KernelPath, Status));

    Status = LoadFileFromSDCard(ImageHandle, SystemTable, KernelPath);
    if (EFI_ERROR(Status))
    {
        Print(L"Failed to load Boot File from path %s: %r\n", KernelPath, Status);
        DEBUG((EFI_D_ERROR, "Failed to load Boot File from path %s: %r\n", KernelPath, Status));
    }
    else
    {
        DEBUG((EFI_D_ERROR, "Boot File loaded successfully from path %s\n", KernelPath));

        BOOT_IMG_HDR *Header = ParseBootImageHeader(BootFile);
        if (Header)
        {
            DEBUG((EFI_D_ERROR, "Image is an Android Bootimage!\n"));
            ...
            DEBUG((EFI_D_ERROR, "KernelLoadAdress is: %p, RamdiskLoadAdress is: %p\n", KernelLoadAddress, RamdiskLoadAddress));
            // somehow load only the kernel and ramdisk here to the correct adresses read from bootimage
            LoadBootImageParts(Header, BootFile, &KernelBuffer, &RamdiskBuffer);
            ...
            CmdlineSize = AsciiStrLen(Header->cmdline) + 1; // +1 for the null terminator
            ...
            else
            {
                ...
                BootLinux(ImageHandle, SystemTable, KernelBuffer, &Header->tags_addr, AllocatedCmdline, MachType, RamdiskBuffer, Header->ramdisk_size);
            }
        }
        else
        {
            ...
            {
                DEBUG((EFI_D_ERROR, "Image loading into address %p\n", KernelLoadAddress));
                goto cleanup;
            }
            else
            {
                DEBUG((EFI_D_ERROR, "Image loaded into memory adress %p. Size: %d bytes\n", KernelBuffer, KernelSize));
            }
            DEBUG((EFI_D_ERROR, "Image is not an Android Bootimage, testing for zImage\n"));
            ...
            {
                DEBUG((EFI_D_ERROR, "Bad Linux ARM zImage magic, booting only the image\n"));
            ...
            if (Status != EFI_SUCCESS)
            {
                DEBUG((EFI_D_ERROR, "Ramdisk loading into address %p failed\n", KernelLoadAddress));
                goto cleanup;
            }
            else
            {
                DEBUG((EFI_D_ERROR, "Ramdisk loaded into memory adress %p. Size: %d bytes\n", KernelBuffer, KernelSize));
            }
            // Allocate memory for cmdline
